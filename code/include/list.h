 ...
 * Iterates over list.
 */
#define ccdListForEach(list, item) for (item = (list)->next; _ccd_prefetch((item)->next), item != (list); item = (item)->next)
...
#define ccdListForEachEntry(head, pos, postype, member)                                                              \
    for (pos = ccdListEntry((head)->next, postype, member); _ccd_prefetch(pos->member.next), &pos->member != (head); \
...
#define ccdListForEachEntrySafe(head, pos, postype, n, ntype, member)                                            \
    for (pos = ccdListEntry((head)->next, postype, member), n = ccdListEntry(pos->member.next, postype, member); \
         &pos->member != (head); pos = n, n = ccdListEntry(n->member.next, ntype, member))
