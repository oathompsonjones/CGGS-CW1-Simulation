 ...
 *  vertices. */
static int simplexToPolytope4(const void *obj1, const void *obj2, const ccd_t *ccd, ccd_simplex_t *simplex, ccd_pt_t *pt,
                              ccd_pt_el_t **nearest);

/** Transforms simplex to polytope, three vertices required */
static int simplexToPolytope3(const void *obj1, const void *obj2, const ccd_t *ccd, const ccd_simplex_t *simplex, ccd_pt_t *pt,
                              ccd_pt_el_t **nearest);

/** Transforms simplex to polytope, two vertices required */
static int simplexToPolytope2(const void *obj1, const void *obj2, const ccd_t *ccd, const ccd_simplex_t *simplex, ccd_pt_t *pt,
                              ccd_pt_el_t **nearest);
...
int ccdGJKSeparate(const void *obj1, const void *obj2, const ccd_t *ccd, ccd_vec3_t *sep) {
    ...
    if (nearest) ccdVec3Copy(sep, &nearest->witness);
...
static int __ccdGJKEPA(const void *obj1, const void *obj2, const ccd_t *ccd, ccd_pt_t *polytope, ccd_pt_el_t **nearest) {
    ccd_simplex_t simplex;
    ...
    *nearest = NULL;
    ...
    while (1) {
        ...
        *nearest = ccdPtNearest(polytope);
        ...
        if (nextSupport(obj1, obj2, ccd, *nearest, &supp) != 0) break;
...
static int simplexToPolytope2(const void *obj1, const void *obj2, const ccd_t *ccd, const ccd_simplex_t *simplex, ccd_pt_t *pt,
    ...
    if (!found) goto simplexToPolytope2_touching_contact;

    // get second support point in opposite direction than supp[0]
    ccdVec3Copy(&dir, &supp[0].v);
    ccdVec3Scale(&dir, -CCD_ONE);
    __ccdSupport(obj1, obj2, &dir, ccd, &supp[1]);
    if (ccdVec3Eq(&a->v, &supp[1].v) || ccdVec3Eq(&b->v, &supp[1].v)) goto simplexToPolytope2_touching_contact;
    ...
    if (ccdVec3Eq(&a->v, &supp[2].v) || ccdVec3Eq(&b->v, &supp[2].v)) goto simplexToPolytope2_touching_contact;

    // and last one will be in opposite direction
    ccdVec3Scale(&dir, -CCD_ONE);
    __ccdSupport(obj1, obj2, &dir, ccd, &supp[3]);
    if (ccdVec3Eq(&a->v, &supp[3].v) || ccdVec3Eq(&b->v, &supp[3].v)) goto simplexToPolytope2_touching_contact;
...
static int nextSupport(const void *obj1, const void *obj2, const ccd_t *ccd, const ccd_pt_el_t *el, ccd_support_t *out) {
    ...
    if (dist - el->dist < ccd->epa_tolerance) return -1;
    ...
    if (dist < ccd->epa_tolerance) return -1;
