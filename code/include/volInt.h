...
void compFaceIntegrals(const Eigen::MatrixXd& V, const Eigen::RowVectorXi& f, const Eigen::RowVector3d& n) {
    ...
    Fbbb = k1 * Pbbb;
    Fccc =
        -k4 * (CUBE(n(A)) * Paaa + 3 * SQR(n(A)) * n(B) * Paab + 3 * n[A] * SQR(n(B)) * Pabb + CUBE(n(B)) * Pbbb +
...
void getCOMandInvIT(const Eigen::MatrixXd& V, const Eigen::MatrixXi& T, const double density, double& mass, Eigen::RowVector3d& COM,
                    Eigen::Matrix3d& invIT) {
